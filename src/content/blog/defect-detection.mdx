...
abstract: "In this project, I will attempt to identify regions of defects in lattice structures. By using techniques like Delaunay triangulation and Voronoi tessellation, estimate coordination number and coordination sphere to pinpoint the areas of defects."
pubDate: "Jul 02 2022"
...
Given the image for a lattice structure, it might be easy to visually detect some of the glaring defects present in the material, but it's not always so easy to pinpoint some of the areas of fault. It becomes even more difficult to be able to this algorithmically.

The problem becomes easier for humans to detect these defects if a video of the heat-treatment process is provided, as we are able to better pinpoint areas that change significantly through time.

In this blog post, I'd like to showcase one of the methods I used to attempt this problem.

## Source Material

For the problem, a excelent video by Steve Mould[^steve-channel] came across my YouTube feed.
...
In this video, Steve Mould shows what happens when a material is heat-treated and how the amount of defect present in a material can be reduced through this process.
...
Initially, a high temperature (more vibrations) is set, which was gradually decreased, simulating the heat-treatment/annealing process of materials. From his video, it was easy to see that the amount of defects, be it point or line defects, were reduced gradually as the treatment progressed.
...
Due to the artifacts identified before, there was some offset between this identified center and true center of the atom. As the artifacts on the atoms were similar, the offsets were also the same for all atoms. As correctly identifying the offset was challenging, and the offsets were small, I chose to ignore the offsets.
...
The coordination number represents the number of bonds each atom has. By analyzing the coordination number, we can identify regions where the coordination deviated from the global median, indicating potential defects in the material.

One stratergy could be use the centers identified earlier and consider all the atoms within a distance of 1.5 times the diameter as neighbors. However, this approach is not practical, as it assumes a perfect crystalline structure and would not work effectively near regions of defects.
...
- for defects where a line of atoms is slightly displaced, the coordination number will remain the same as in defectless regions, which can result in the entire region being incorectly identified as defectless.

## 3. Coordination Sphere

The coordination sphere refers to the region of influence of each atom, and its deviation in volume (or area) can be used to identify regions of defects. At the regions of defects, due to inoptimal packing the area will increase, alerting us about the defect.
